...
class RedisSessionStore(DeltaSessionStore):
    ...
    def _serialize(self, session: DeltaSession) -> str:
        data = asdict(session)
    ...
    def _deserialize(self, data_str: str) -> DeltaSession:
        ...
        if data.get("user_info") is not None:
            data["user_info"] = DeltaUserInfo(**data["user_info"])
        return DeltaSession(**data)
